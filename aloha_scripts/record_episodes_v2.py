...
from aloha_scripts.RemoteMasterBot import RemoteMasterBot
from constants import DT, START_ARM_POSE, TASK_CONFIGS, MASTER_GRIPPER_JOINT_NORMALIZE_FN
...
from real_env import make_real_env, get_action, get_action_from_joint_states
...
def opening_ceremony_master(master_bot_left, master_bot_right):
    ...
    # move_grippers([master_bot_left, master_bot_right], [PUPPET_GRIPPER_JOINT_CLOSE] * 2, move_time=0.5)
    ...
    while not pressed:
        ...
        if (gripper_pos_left < close_thresh) and (gripper_pos_right < close_thresh):
            pressed = True
        time.sleep(DT/10)
    master_bot_left.torque_off()
    master_bot_right.torque_off()
    print(f'Started!')
...
def capture_one_episode(dt, max_timesteps, camera_names, dataset_dir, dataset_name, overwrite):
    ...
    opening_ceremony_puppet(env.puppet_bot_left, env.puppet_bot_right)
    opening_ceremony_master(master_bot_left, master_bot_right)
